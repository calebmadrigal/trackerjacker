...
class Trigger:
    ...
    def show_top_devices(self):
        top_devices = []
        for power, dev_id in heapq.nlargest(TOP_N_TO_SHOW,
    ...
    def decay_items(self):
        ...
        if self.frame_count % 100 != 0 or len(self.dev_to_power) <= TOP_N_TO_SHOW:
            return  # Don't bother removing items if we are under the limit

        num_items_to_remove = len(self.dev_to_power) - TOP_N_TO_SHOW
        for last_seen, dev_id in heapq.nsmallest(num_items_to_remove,
